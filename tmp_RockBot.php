...
class RockBot {
    ...
    //private $parser_link = 'http://159.89.18.171:3001/';
    private $parser_link = 'https://5anffarj00.execute-api.eu-central-1.amazonaws.com/';
    private $music_resources = array(
        ...
        'music.youtube' => array(
            'name' => 'YouTube music', 'db_field' => 'music_youtube', 'parser_name' => 'youtube', 'image' => 3, 'format' => " ♪ ", 'default' => 1,
        ...
        'spotify.com' => array(
            'name' => 'Spotify', 'db_field' => 'spotify', 'parser_name' => 'spotify', 'image' => 2, 'format' => " ♪ ", 'default' => 1,
            'search_link' => 'https://open.spotify.com/search/{search_text}',
        ),
        'deezer.com' => array(
            'name' => 'Deezer', 'db_field' => 'deezer', 'parser_name' =>'deezer', 'image' => 1, 'format' => " ♪ ", 'default' => 1,
            'search_link' => 'https://www.deezer.com/search/{search_text}',
        ),
        'last.fm' => array(
            'name' => 'last.fm', 'db_field' => 'last_fm', 'parser_name' => 'lastfm', 'image' => 4, 'format' => " ♪ ",
        ...
        // закоментируй parser_name чтоб не делало запрос на парсинг ни в очереди предварительного парсера ни когда прередаешь название релиза
        //'vk.ru' => array('name' => 'vk', 'link' => 'https://vk.ru/novue_rock_albomu_2013', 'format' => " ♪ "),
    );
    private $types = [
        'a' => 'album',
        'а' => 'album',
    ...
    private function processMessage() {
        ...
        if ($text == '/start') {
            ...
            $is_save = false;
            if (mb_strpos($text, '/qq') === 0) {
                $text = str_replace('/qq', '', $text);
                $is_save = true; // add to DB for queue parsing to get cache
            }
            $links = explode("\n", str_replace('/q', '', $text));
            ...
            foreach ($links as $l) {
                ...
                if ($sep !== false) {
                    // delete emojies
                    $l = trim(preg_replace('/^[\x{1F300}-\x{1FAFF}\x{2600}-\x{27BF}\s]+|[\x{1F300}-\x{1FAFF}\x{2600}-\x{27BF}\s]+$/u', '', $l));
...
//todo $this->vkPost($post_text, $delay_date);
    ...
    {
        $ret = array();
        $a = "https://api.vk.ru/method/audio.getUploadServer?&access_token={$this->settings['vk_token']}&v=" . self::VERSION_VK;
        ...
        foreach ($audioFiles as $file) {
            ...
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, 'https://api.vk.ru/method/audio.save');
    ...
    private function processChatMessage()
	{
        if (!empty($this->result['message']['from']['id']) && $this->result['message']['from']['id'] == 777000) {
            return; // for forwarded from main channel
        }

        $json_result = mb_strtolower(json_encode($this->result, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE));
        if  (
            mb_stripos($json_result, 'jetton') !== FALSE
            || mb_stripos($json_result, 'тамаев') !== FALSE
            || mb_stripos($json_result, 'tamaev') !== FALSE
            || mb_stripos($json_result, 'mellstroy') !== FALSE
            || mb_strpos($json_result, '@wamanager') !== FALSE
            || mb_strpos($json_result, '@iliyuh') !== FALSE
            || mb_strpos($json_result, '@liyuh') !== FALSE
            || mb_strpos($json_result, '@ilya') !== FALSE
            || mb_strpos($json_result, 'jetgame') !== FALSE
            || mb_strpos($json_result, '@trumpcoln') !== FALSE
            || mb_strpos($json_result, '@pravalleksander') !== FALSE
            || mb_strpos($json_result, 'tonhunt') !== FALSE
            || mb_strpos($json_result, '@docsotzyvy') !== FALSE
            || mb_strpos($json_result, '@easyprimejob') !== FALSE
            || mb_strpos($json_result, '@neural') !== FALSE
            || mb_strpos($json_result, '@katyhr') !== FALSE
            || mb_strpos($json_result, 'nuero') !== FALSE
            || mb_strpos($json_result, 'neuro') !== FALSE
            || mb_strpos($json_result, '@ivest') !== FALSE
            || mb_strpos($json_result, '@anonchates') !== FALSE
            || mb_strpos($json_result, '@victorhr') !== FALSE
            || mb_strpos($json_result, 'semirov') !== FALSE
            || mb_strpos($json_result, 'levchnenko') !== FALSE
            || mb_strpos($json_result, 'plotnikova') !== FALSE
            || mb_strpos($json_result, '@dmitriy_as') !== FALSE
            || mb_strpos($json_result, '@daff') !== FALSE
            || mb_strpos($json_result, '@operator') !== FALSE
            || mb_strpos($json_result, '@diana_isaeva') !== FALSE
            || mb_strpos($json_result, '@fpi') !== FALSE
            || mb_strpos($json_result, '@leon_sp') !== FALSE
            || mb_strpos($json_result, '@tgsuper') !== FALSE
            || mb_strpos($json_result, '@topwork') !== FALSE
            || mb_strpos($json_result, '@funnyland') !== FALSE
            || mb_strpos($json_result, '@fullwork') !== FALSE
            || mb_strpos($json_result, '@darkwaiders') !== FALSE
            || mb_strpos($json_result, 'mikhail_shiryaev') !== FALSE
            || mb_strpos($json_result, '@izolenta36') !== FALSE
            || mb_strpos($json_result, '@fullwork') !== FALSE
            || mb_strpos($json_result, '@vrn36karty') !== FALSE
            || mb_strpos($json_result, '@eurnnquuBot') !== FALSE
            || mb_strpos($json_result, 'parik24.com') !== FALSE
            || mb_strpos($json_result, 'keonv.com') !== FALSE
            || mb_strpos($json_result, 'kutt.it') !== FALSE
            || mb_strpos($json_result, 'femaleai.great-site.net') !== FALSE
            || mb_strpos($json_result, 'cashbot.gt.tc') !== FALSE
            || mb_strpos($json_result, 'interdicting.rf.gd') !== FALSE
            || mb_strpos($json_result, 'money-gamble.totalh.net') !== FALSE
            || mb_strpos($json_result, 'Thee_best_winsingsBot') !== FALSE
            || mb_strpos($json_result, 'играю тут -') !== FALSE
            || mb_strpos($json_result, 'bedar_beas') !== FALSE
            || mb_strpos($json_result, 'фриспин') !== FALSE
            || mb_strpos($json_result, 'luxury ai') !== FALSE
            || mb_strpos($json_result, 'trc20') !== FALSE
            || mb_strpos($json_result, 'юсдт') !== FALSE
            || mb_strpos($json_result, 'д€ᛠርkθ€') !== FALSE
            || mb_strpos($json_result, 'vjsnuxisbot') !== FALSE
            || mb_strpos($json_result, 'usnxusiwnbot') !== FALSE
            || mb_strpos($json_result, 'dnshxjvobot') !== FALSE
            || mb_strpos($json_result, '3apa6') !== FALSE // транслит (заработок)
            || (mb_stripos($json_result, 'inline_keyboard') !== FALSE && mb_stripos($json_result, 'forward_date'))
            || (mb_stripos($json_result, '@') !== FALSE && mb_stripos($json_result, 'prava'))
            || (mb_stripos($json_result, '@') !== FALSE && mb_stripos($json_result, 'casino'))
            || (mb_stripos($json_result, '@') !== FALSE && mb_stripos($json_result, 'atb'))
            || (mb_stripos($json_result, '@') !== FALSE && mb_stripos($json_result, 'epicent'))
            || (mb_stripos($json_result, '@') !== FALSE && mb_stripos($json_result, 'cubesolution'))
            || (mb_stripos($json_result, '@') !== FALSE && mb_stripos($json_result, 'luckywin'))
            || (mb_stripos($json_result, 'portals') !== FALSE && mb_stripos($json_result, 'маркетплейс'))
            || (mb_stripos($json_result, 'http') !== FALSE && mb_stripos($json_result, 'gt.tc'))
            || (mb_stripos($json_result, 'игpa') !== FALSE && mb_stripos($json_result, 'http'))// английский транслит
            || (mb_stripos($json_result, 'игра') !== FALSE && mb_stripos($json_result, 'http'))
            || (mb_stripos($json_result, 'chatroom') !== FALSE && mb_stripos($json_result, 'http'))
            || (mb_stripos($json_result, 'игра') !== FALSE && mb_stripos($json_result, '@'))
            || (mb_stripos($json_result, 'trust') !== FALSE && mb_stripos($json_result, 'wallet'))
            || (mb_stripos($json_result, 'кᴀзин') !== FALSE && mb_stripos($json_result, 'грн'))
            || (mb_stripos($json_result, 'легальн') !== FALSE && mb_stripos($json_result, 'права'))
            || (mb_stripos($json_result, 'доллар') !== FALSE && mb_stripos($json_result, ' неделю'))
            || (mb_stripos($json_result, 'доллар') !== FALSE && mb_stripos($json_result, ' день'))
            || (mb_stripos($json_result, 'доллар') !== FALSE && mb_stripos($json_result, ' час'))
            || (mb_stripos($json_result, '$') !== FALSE && mb_stripos($json_result, ' неделю'))
            || (mb_stripos($json_result, '$') !== FALSE && mb_stripos($json_result, ' день'))
            || (mb_stripos($json_result, '$') !== FALSE && mb_stripos($json_result, ' час'))
            || (mb_stripos($json_result, 'руб') !== FALSE && mb_stripos($json_result, 'неделю'))
            || (mb_stripos($json_result, 'руб') !== FALSE && mb_stripos($json_result, ' день'))
            || (mb_stripos($json_result, 'руб') !== FALSE && mb_stripos($json_result, ' час'))
            || (mb_stripos($json_result, '80/20') !== FALSE && mb_stripos($json_result, '%'))
            || (mb_stripos($json_result, '$') !== FALSE && mb_stripos($json_result, 'доход'))
            || (mb_stripos($json_result, 'доллар') !== FALSE && mb_stripos($json_result, 'доход'))
            || (mb_stripos($json_result, 'рублей') !== FALSE && mb_stripos($json_result, 'доход'))
            || (mb_stripos($json_result, 'pa6oт') !== FALSE && mb_stripos($json_result, ' лuчн'))// английский транслит
            || (mb_stripos($json_result, 'ραбοт') !== FALSE && mb_stripos($json_result, ' личн'))// английский транслит
            || (mb_stripos($json_result, 'онлайн') !== FALSE && mb_stripos($json_result, ' лс'))
            || (mb_stripos($json_result, 'онлайн') !== FALSE && mb_stripos($json_result, ' личн'))
            || (mb_stripos($json_result, 'онлайн') !== FALSE && mb_stripos($json_result, ' личк'))
            || (mb_stripos($json_result, 'работ') !== FALSE && mb_stripos($json_result, ' л.с'))
            || (mb_stripos($json_result, 'работ') !== FALSE && mb_stripos($json_result, ' лс'))
            || (mb_stripos($json_result, 'работ') !== FALSE && mb_stripos($json_result, ' личн'))
            || (mb_stripos($json_result, 'работ') !== FALSE && mb_stripos($json_result, ' личк'))
            || (mb_stripos($json_result, 'paбoт') !== FALSE && mb_stripos($json_result, ' лс'))   // английский транслит
            || (mb_stripos($json_result, 'paбoт') !== FALSE && mb_stripos($json_result, ' личн')) // английский транслит
            || (mb_stripos($json_result, 'paбoт') !== FALSE && mb_stripos($json_result, ' личк')) // английский транслит
            || (mb_stripos($json_result, 'доллар') !== FALSE && mb_stripos($json_result, ' лс'))
            || (mb_stripos($json_result, 'доллар') !== FALSE && mb_stripos($json_result, ' личн'))
            || (mb_stripos($json_result, 'доллар') !== FALSE && mb_stripos($json_result, ' личк'))
            || (mb_stripos($json_result, '$') !== FALSE && mb_stripos($json_result, ' лс'))
            || (mb_stripos($json_result, '$') !== FALSE && mb_stripos($json_result, ' личн'))
            || (mb_stripos($json_result, '$') !== FALSE && mb_stripos($json_result, ' личк'))
            || (mb_stripos($json_result, 'руб') !== FALSE && mb_stripos($json_result, ' лс'))
            || (mb_stripos($json_result, 'руб') !== FALSE && mb_stripos($json_result, ' личн'))
            || (mb_stripos($json_result, 'руб') !== FALSE && mb_stripos($json_result, ' личк'))
            || (mb_stripos($json_result, 'получ') !== FALSE && mb_stripos($json_result, ' лс'))
            || (mb_stripos($json_result, 'получ') !== FALSE && mb_stripos($json_result, ' л.с'))
            || (mb_stripos($json_result, 'получ') !== FALSE && mb_stripos($json_result, ' личн'))
            || (mb_stripos($json_result, 'получ') !== FALSE && mb_stripos($json_result, ' личк'))
            || (mb_stripos($json_result, 'доход') !== FALSE && mb_stripos($json_result, ' лс'))
            || (mb_stripos($json_result, 'доход') !== FALSE && mb_stripos($json_result, ' личн'))
            || (mb_stripos($json_result, 'доход') !== FALSE && mb_stripos($json_result, ' личк'))
            || (mb_stripos($json_result, 'доход') !== FALSE && mb_stripos($json_result, ' л.с'))
            || (mb_stripos($json_result, 'прибыль') !== FALSE && mb_stripos($json_result, ' личн'))
            || (mb_stripos($json_result, 'прибыль') !== FALSE && mb_stripos($json_result, ' личк'))
            || (mb_stripos($json_result, 'прибыль') !== FALSE && mb_stripos($json_result, ' личк'))
            || (mb_stripos($json_result, 'прибыль') !== FALSE && mb_stripos($json_result, ' л.с'))
            || (mb_stripos($json_result, 'прибыль') !== FALSE && mb_stripos($json_result, ' зараб'))
            || (mb_stripos($json_result, 'удал') !== FALSE && mb_stripos($json_result, ' лс') !== FALSE)
            || (mb_stripos($json_result, 'удал') !== FALSE && mb_stripos($json_result, 'личн') !== FALSE)
            || (mb_stripos($json_result, 'удал') !== FALSE && mb_stripos($json_result, 'личк') !== FALSE)
            || (mb_stripos($json_result, 'удал') !== FALSE && mb_stripos($json_result, 'работ') !== FALSE)
            || (mb_stripos($json_result, 'ставки') !== FALSE && mb_stripos($json_result, 'спорт') !== FALSE)
            || (mb_stripos($json_result, 'доступным') !== FALSE && mb_stripos($json_result, 'ценам') !== FALSE)
            || (mb_stripos($json_result, '$') !== FALSE && mb_stripos($json_result, 'пиши') !== FALSE)
            || (mb_stripos($json_result, 'https://') !== FALSE && mb_stripos($json_result, 'κазин'))//транслит
            || (mb_stripos($json_result, 'https://') !== FALSE && mb_stripos($json_result, 'κaзин'))//транслит
            || (mb_stripos($json_result, 'https://') !== FALSE && mb_stripos($json_result, 'кaзuн'))//транслит
            || (mb_stripos($json_result, 'https://') !== FALSE && mb_stripos($json_result, 'ᴋазин'))//транслит
            || (mb_stripos($json_result, 'https://') !== FALSE && mb_stripos($json_result, 'ᴋазuн'))//транслит
            || (mb_stripos($json_result, 'https://') !== FALSE && mb_stripos($json_result, 'ка3ик'))//транслит
            || (mb_stripos($json_result, 'купить') !== FALSE && mb_stripos($json_result, 'звезд') !== FALSE)
            || (mb_stripos($json_result, 'куп') !== FALSE && mb_stripos($json_result, 'usdt') !== FALSE)
            || (mb_stripos($json_result, 'каз') !== FALSE && mb_stripos($json_result, 'http'))
            //|| (mb_stripos($json_result, 'игра') !== FALSE && mb_stripos($json_result, '.net') && mb_stripos($json_result, 'https://'))
            || (mb_stripos($json_result, 'работ') !== FALSE && mb_stripos($json_result, ' л.') && mb_stripos($json_result, 'с.'))
            || (mb_stripos($json_result, 'работ') !== FALSE && mb_stripos($json_result, 'онлайн') && mb_stripos($json_result, 'знакомств'))
            || (mb_stripos($json_result, 'проверь') !== FALSE && mb_stripos($json_result, 'интим') && mb_stripos($json_result, 'фото'))
            || (mb_stripos($json_result, 'сделк') !== FALSE && mb_stripos($json_result, 'трейдер') !== FALSE && mb_stripos($json_result, 'торгов') !== FALSE)
            || (mb_stripos($json_result, 'команд') !== FALSE && mb_stripos($json_result, 'человек') !== FALSE && mb_stripos($json_result, 'телефон') !== FALSE)
            || (mb_stripos($json_result, 'одежд') !== FALSE && mb_stripos($json_result, 'цен') !== FALSE && mb_stripos($json_result, 'бренд') !== FALSE)
            || (mb_stripos($json_result, 'онлайн') !== FALSE && mb_stripos($json_result, 'присоединяйтесь') && mb_stripos($json_result, 'доход'))
		...
		if (!empty($this->result['message']['forward_date'])) {
            ...
            $title = '';
             if (!empty($this->result['message']['forward_origin']['chat']['title'])) {
                $title = $this->result['message']['forward_origin']['chat']['title'];
            }
            ...
            if (
                mb_strpos($text, 'кази') !== FALSE
                || mb_strpos($text, 'mell ') !== FALSE
                || mb_strpos($text, 'cаsinо') !== FALSE // mixed latin-kiril
                || mb_strpos($text, 'casino') !== FALSE
                || mb_strpos($text, 'бонус') !== FALSE
                || mb_strpos($text, 'звёзд') !== FALSE
                || mb_strpos($text, 'звезд') !== FALSE
                || mb_strpos($text, 'крипт') !== FALSE
                || mb_strpos($text, 'игра') !== FALSE
                || mb_strpos($text, 'игpa') !== FALSE// английский транслит
                || mb_strpos($text, 'зевс') !== FALSE
                || mb_strpos($text, 'залет') !== FALSE
                || mb_strpos($text, 'лимон') !== FALSE
                || mb_strpos($text, ' лям') !== FALSE
                || mb_strpos($text, 'кручу') !== FALSE
                || mb_strpos($text, 'сыпет') !== FALSE
                || mb_strpos($text, 'слоты') !== FALSE
                || mb_strpos($text, 'kritvbs') !== FALSE
                || mb_strpos($text, 'ночные бабочки') !== FALSE
                || mb_strpos($text, 'раздев') !== FALSE
                || mb_strpos($text, 'разден') !== FALSE
                || mb_strpos($text, 'секс') !== FALSE
                || mb_strpos($text, 'интим') !== FALSE
                || mb_strpos($text, 'uнтum') !== FALSE
                || mb_strpos($text, 'обнаженные') !== FALSE
                || mb_strpos($text, 'проститутк') !== FALSE
                || mb_strpos($text, 'взрослы') !== FALSE
                || mb_strpos($text, 'малышка') !== FALSE
                || mb_strpos($text, 'трах') !== FALSE
                || mb_strpos($text, '18') !== FALSE
                || mb_strpos($title, '18') !== FALSE
                || mb_strpos($text, 'vpn') !== FALSE
                || mb_strpos($title, 'vpn') !== FALSE
                || mb_strpos($text, 'впн') !== FALSE
                || mb_strpos($title, 'впн') !== FALSE
                || mb_strpos($title, 'vip') !== FALSE
                || mb_strpos($text, 'знакомств') !== FALSE
                || mb_strpos($text, 'сучк') !== FALSE
                || mb_strpos($text, 'luxury ai') !== FALSE
                || mb_strpos($text, 'покер') !== FALSE
                || mb_strpos($text, 'bonanza') !== FALSE
                || mb_strpos($text, 'lucky bear') !== FALSE
                || mb_strpos($text, 'luckybear') !== FALSE
                || mb_strpos($text, 'luckywin') !== FALSE
                || mb_strpos($text, '@ton') !== FALSE
                || mb_strpos($text, '@miningllc') !== FALSE
            ) {
                $this->telegram->sendAnyRequest('deleteMessage', ['chat_id' => $this->chat_id, 'message_id' => $this->result['message']['message_id']]);
            }
        }
    ...
    private function getPostAudio(&$postData, $audio_channel_id)
    {
        ...
        if (!empty($audios)) {
            ...
            $type_album = $postData['type_album'];
            if ($postData['strict_type'] == 0) {
                if ($cAudios > 7) {
                    $type_album = $postData['type_album'] = 'album';
                } elseif ($cAudios > 3) {
                    $type_album = $postData['type_album'] = 'EP';
                }
            }
            if (strpos($type_album, ' ') === false) {
    ...
    private function queueParse()
    {
        ...
        $neededLinks = [];
        foreach ($this->music_resources as $mr) {
            if (!empty($mr['db_field']) && !empty($mr['parser_name']) && $mr['parser_name'] == 'spotify') { // TODO: parse only spotify with queue because it's parsed with browser. To parse all remove $mr['parser_name'] == 'spotify' 
